...
function gnntarget(model, ds, classes::Vector{Int})
    d = size(model(ds), 1)
    Flux.onehotbatch(classes, 1:d)
end

function gnntarget(model, ds, class::Int)
    gnntarget(model, ds, fill(class, numobs(ds)))
end
...
function ChainRulesCore.rrule(::typeof(entropy), x::AbstractArray)
    @assert all((x.>= 0) .& (x .<=1))
    return _entropy(x), Δ -> (NoTangent(), _∇entropy(Δ, x)...)
end
