...
Mask(ds::ArrayNode{T,M}, m::ArrayModel) where {T<:Matrix, M} =  Mask(ds)

function prune(ds::ArrayNode{T,M}, m::MatrixMask, cluster_algorithm = cluster_instances) where {T<:Matrix, M}
	x = deepcopy(ds.data)
	x[.!mask(m), :] .= 0
