...
function sigmoid(d::Dense, α, xmin, xmax)
	@warn "sigmoid for dense with $(d.σ) not implemented"
	d
end
...
function sigmoid(m::BagModel, α, x::BagNode)
	...
	c_xmin, c_xmax = minimax(m.a(imσ(x.data), x.bags))
	fuseaffine!(m.bm, c_xmin, c_xmax, n_xmin, n_xmax)
	BagModel(imσ, m.a, m.bm)
end

function sigmoid(m::ProductModel, α, x::TreeNode)
	n_xmin, n_xmax = minimax(ProductModel(m.ms, identity)(x))
	mσ = map(k -> k => sigmoid(m.ms[k], α, x.data[k]), keys(m.ms))
	mσ = (;mσ...)
	c_xmin, c_xmax = minimax(ProductModel(mσ, identity)(x))
	@show (n_xmin, n_xmax)
	@show (c_xmin, c_xmax)
